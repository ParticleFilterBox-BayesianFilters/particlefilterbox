...
class TestMultinomialResample:
    def test_correct_number_of_indices(self) -> None:
        w = np.ones(100) / 100
        indices = multinomial_resample(w, rng=np.random.default_rng(42))
    ...
    def test_reproducibility(self) -> None:
        w = np.ones(100) / 100
        idx1 = multinomial_resample(w, rng=np.random.default_rng(42))
        idx2 = multinomial_resample(w, rng=np.random.default_rng(42))
    ...
    def test_preserves_weighted_mean(self) -> None:
        ...
        indices = multinomial_resample(w, rng=rng)
